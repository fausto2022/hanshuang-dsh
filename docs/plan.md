...
> 本插件由 DSH 插件开发助手（dsh-plugin-studio）在既有牛来仓库上扩展。
> 每阶段决策确定后勾选对应项，未通过不得进入下一阶段。

## 阶段 ①：需求捕获

- [x] 插件名：`dsh-niulai`
- [x] 一句话目标：在 DSH Web 设置里选择、修改、导入牛来系统提示词，并立即注入后续对话
- [x] 能力面清单：
- [x] 命令/工具（保留 `/niulai`）
- [x] HTTP 接口（`/dsh-niulai/*` 配置读写）
- [x] 设置面板（Settings → 牛来）
- [x] 目标 profile：web
...
- [x] `src/client/index.ts` 已生成（settings.section 牛来页）
- [x] `inject` 已覆盖所有服务（node 顶层: `systemPrompt`；`webServer` / `command` 用嵌套 `ctx.inject` 可选等待；client: `slots`）
- [x] 冒烟功能就绪（`/niulai` 有 command 时注册、`GET /dsh-niulai/health`、设置页「牛来」）
...
- 在既有 `dsh-niulai` 仓库上扩展，而不是新建空项目。
- 设置页注册真实 slot `settings.section`（id=`niulai`），而不是过时的 `settings`。
