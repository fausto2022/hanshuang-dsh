...
var PERSONA_PREFIX_NAMES = /* @__PURE__ */ new Set([
  "deployment:persona-prefix",
  "persona-prefix"
]);
var PERSONA_LEGACY_NAMES = /* @__PURE__ */ new Set(["deployment:persona", "persona"]);
function stripHarnessPersona(text) {
  if (!text) return text;
  return text.replace(/You are an AI agent powered by DeepSeek Harness\.?\s*/g, "").replace(/You are a helpful software engineer assistant\.?\s*/gi, "").replace(/你是一个有帮助的软件工程师助手[。.]?\s*/g, "").replace(/我是 DeepSeek(?:驱动)?的? AI(?:编程)?助手[，,]?[^\n]*/g, "").replace(/我是 DeepSeek[^\n]*/g, "").replace(/由深度求索[^\n]*/g, "").replace(/你好！?我是 DeepSeek[^\n]*/g, "").replace(
    ...
    ""
  ).replace(/You are a coding agent powered by the [^\n.]+ model\.?\s*/g, "").replace(/[ \t]{2,}/g, " ").replace(/ \n/g, "\n").trim();
}
function collectInjectText(niulaiSections, fallbackText) {
  const fromSections = niulaiSections.map((section) => String(section.text || "").trim()).filter(Boolean).join("\n\n").trim();
  if (fromSections) return fromSections;
  return typeof fallbackText === "string" ? fallbackText.trim() : "";
}
function alreadyFolded(prior, inject2) {
  if (!inject2) return true;
  const needle = inject2.slice(0, Math.min(80, inject2.length));
  return Boolean(needle) && prior.includes(needle);
}
function isPersonaFoldTarget(name2, preferPrefix) {
  if (preferPrefix) return PERSONA_PREFIX_NAMES.has(name2);
  return PERSONA_PREFIX_NAMES.has(name2) || PERSONA_LEGACY_NAMES.has(name2);
}
function foldInjectIntoPersona(sections, injectText) {
  const inject2 = typeof injectText === "string" ? injectText.trim() : "";
  const names = sections.map((section) => String(section?.name || ""));
  const hasPrefix = names.some((name2) => PERSONA_PREFIX_NAMES.has(name2));
  let folded = false;
  const out = sections.map((section) => {
    const name2 = String(section?.name || "");
    if (!isPersonaFoldTarget(name2, hasPrefix)) return section;
    folded = true;
    const prior = String(section.text || "").trim();
    if (!inject2) return { ...section, text: prior };
    if (alreadyFolded(prior, inject2)) return { ...section, text: prior };
    return { ...section, text: prior ? `${inject2}

${prior}` : inject2 };
  });
  if (folded) return { sections: out, folded: true };
  if (!inject2) return { sections: out, folded: false };
  return {
    sections: [{ name: "deployment:persona-prefix", text: inject2 }, ...out],
    folded: true
  };
}
function rewritePromptAssembly(assembled, fallbackInject = "") {
  ...
  for (const section of assembled.sections) {
    ...
    const next = { ...section };
    const name2 = String(next.name || "");
    const isNiulai = name2 === SECTION_NAME || name2.startsWith("niulai-");
...
function installIdentityOverride(ctx, getInjectText) {
  ...
  const hook = async (_assembly, _context, next) => {
    ...
    if (!fallback && !assembled?.sections?.some((section) => {
      const name2 = String(section?.name || "");
      return name2 === SECTION_NAME || name2.startsWith("niulai-");
    })) {
      return assembled;
    }
