...
注入开启时会剥掉官方 Harness 身份句，并把牛来段折进 persona，避免被梁神 phase-1 滤掉。设置页关掉或 `/niulai off` 后，官方身份完整回来。

设置页冒烟：Settings 左侧出现 **牛来**，能看到 v3 / flash / variant-b / astra 四张卡片。
...
| 变体 | 适用模型 | 语言 | 特点 |
|------|----------|------|------|
| v3（默认） | 中高端模型 | 中英混合 | 完整工程规范、硬拒绝场景覆盖、拒绝话术全清单、任务优先 / 硬输出 / 缺值占位 / 稳定锚 |
